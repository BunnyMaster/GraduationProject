...
// TODO 修改内容
Router.post("/ChangeTodolistCountent", (request: any, response: any) => {
  ...
  //  语句
  let statement: string = `UPDATE graduation.todolist SET ${startvalue}  WHERE (id = '${body.id}' and uuid='${body.uuid}');`;
  ...
  MySQL.query(statement, (Error: any, result: any) => {
    if (Error) {
      ...
      response.send(data);
      console.log("POST /api/equipment/ChangeTodolistCountent---错误");
    } else {
      let data: object = {
        code: 200,
        message: "修改成功",
      };
      console.log("POST /api/equipment/ChangeTodolistCountent---成功");
      response.send(data);
    }
  });
});
