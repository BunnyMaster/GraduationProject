      ...
      <el-table :data="tableData" style="width: 100%" stripe :row-class-name="Fun.tableRowClassName" :default-sort="{ prop: 'date', order: 'descending' }" max-height="650">
        <template #empty>
          <el-empty>
            ...
            <el-button type="primary" @click="Fun.GetInfraredCounterGunList()">刷新</el-button>
          </el-empty>
        </template>
        <!--      TODO 设备条码-->
        ...
        <!--   TODO --- 采购时间  -->
        <el-table-column align="center" sortable prop="PurchaseTime" label="采购时间" width="266" style="text-align: center">
          <template #default="scope">
        ...
        <!--  TODO --- 用途   -->
        <el-table-column align="center" prop="SensingWay" label="用途" width="266" />
        ...
        <el-table-column align="center" fixed="right" label="操作" width="166">
          <template #default="scope">
            ...
            <el-popconfirm
              title="此项添加的是随机数据是否继续?"
              confirm-button-text="确认"
              cancel-button-text="返回"
              :icon="Plus"
              icon-color="#F56C6C"
              @confirm="Fun.handleEdit()"
              @cancel="Fun.cancel()"
            >
              <template #reference>
                <el-button type="success" size="small" plain :icon="Plus"> 添加 </el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
  ...
  <el-dialog v-model="dialogFormVisible" :title="`${$route.meta.title}`" draggable="draggable">
    <el-form :model="form">
      ...
      <el-form-item label="设备规格" :label-width="form.formLabelWidth">
        <el-select v-model="form.name.DeviceType" placeholder="请输入设备规格">
          <el-option v-for="(LabelItem, LableIndex) in form.MenuBarList.DevicTtype" :key="LableIndex" :label="LabelItem" :value="LabelItem" />
        </el-select>
      </el-form-item>
      <!--      设备规格-->
      <el-form-item label="感应距离" :label-width="form.formLabelWidth">
        <el-input v-model="form.name.SensingDistance" placeholder="请输入感应距离" style="width: 300px" />
      </el-form-item>
      <!--      采购时间-->
      ...
      <!--      所属工位-->
      <el-form-item label="感应方式" :label-width="form.formLabelWidth">
        <el-input v-model="form.name.ClassFacility" placeholder="请输入感应方式" style="width: 300px" />
      </el-form-item>
      <!--      分辨率-->
      <el-form-item label="采购时间" :label-width="form.formLabelWidth">
        <div class="block" style="width: 300px">
          <el-date-picker v-model="form.name.PurchaseTime" type="datetime" placeholder="请输入采购时间" />
        </div>
      </el-form-item>
      <!--      核心数-->
      <el-form-item label="出厂编号" :label-width="form.formLabelWidth">
        <el-input v-model="form.name.GoNumber" placeholder="请输入出厂编号" style="width: 300px" />
      </el-form-item>
      <!--      内存-->
      <el-form-item label="用途" :label-width="form.formLabelWidth">
        <el-input v-model="form.name.SensingWay" placeholder="请输入用途" style="width: 300px" />
      </el-form-item>
...
const Fun = reactive({
  ...
  async handleDelete(index: number, row: any) {
    ElMessage.closeAll();
    try {
      let result = await store.dispatch("InfraredCounterGunDelete", { id: row.id });
      ElMessage({
        showClose: true,
        message: `${result}`,
        type: "success",
  ...
  async SubmitName() {
    ...
    let change: string = dayjs(data.PurchaseTime).format("YYYY-MM-DD HH:mm:ss");
    data.PurchaseTime = change;
    try {
      await store.dispatch("InfraredCounterGunAddItem", data);
      Fun.GetInfraredCounterGunList();
      ElMessage({
        showClose: true,
        message: `添加成功`,
...
const form = reactive({
  name: {
    ...
    GoNumber: "电子秤",
    SensingWay: "深圳市瑞鸿鑫贸易有限公司",
    DepartmentOfOwnership: "行政办公室",
    PrincipalOfAssets: "水果批发",
  },
  delivery: false,
...
const Search_Add = reactive({
  ...
  GoSearch() {
    if (Search_Add.input !== "" && Search_Add.input !== undefined && Search_Add.input.trim()) {
      store.dispatch("InfraredCounterGunSearch", {
