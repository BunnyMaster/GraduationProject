  ...
  >
    <template #default>
      <el-table :data="tableData" style="width: 100%" stripe :row-class-name="Fun.tableRowClassName" :default-sort="{ prop: 'date', order: 'descending' }" max-height="650">
        ...
        <!--   TODO --- 工序编号  -->
        <el-table-column align="center" prop="ProcessFlowMaintenanceNumber" label="工序编号" width="auto">
          <template #default="scope">
        ...
        <!--      TODO 所属工艺流程-->
        <el-table-column align="center" prop="ProcessMaintenanceNumber" label="流程编号" width="auto">
          <template #default="scope">
            <div style="display: flex; justify-content: center; align-items: center">
              <el-icon><Discount /></el-icon>
              <span style="margin-left: 10px">{{ scope.row.ProcessMaintenanceNumber }}</span>
            </div>
          </template>
        </el-table-column>
        <!--   TODO --- 排序号  -->
        <el-table-column align="center" prop="ProcessMaintenanceNo" label="排序号" width="auto" />
      </el-table>
    </template>
  </HomeTableCommon>
</template>
